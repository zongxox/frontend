...
@Component({
  selector: 'app-ind0210',
  templateUrl: './ind0210.component.html',
  styleUrls: ['./ind0210.component.css']
})
export class Ind0210Component implements OnInit {
 ...
 initForm!:FormGroup;
  ...
  ngOnInit(): void {
   ...
   this.regForm = this.fb.group({
    ...
    this.http.get('http://localhost:8080/OrderShipment/0210/init',{ withCredentials: true }).subscribe({
          next:(res:any)=>{
...
//查詢按鈕
  ...
  const data = {
      ...
      this.http.post('http://localhost:8080/OrderShipment/0210/query',data,{ withCredentials: true }).subscribe({
        next:(res:any)=>{
    ...
    delete(id:string){
        ...
        this.http.get(`http://localhost:8080/OrderShipment/0210/delete/${id}`,{ withCredentials: true }).subscribe({
        next:()=>{
        this.info="刪除成功!!";
        setTimeout(() => {
         this.info = '';
...
insert(){
     ...
     console.log(data);
      this.http.post('http://localhost:8080/OrderShipment/0210/insert',data,{ withCredentials: true }).subscribe({
      next:()=>{
      this.ins="新增成功!!";
      setTimeout(() => {
       this.ins = '';
...
update(){
  this.editModel.quantity = this.selectQuantity;
    this.http.post('http://localhost:8080/OrderShipment/0210/update',this.editModel,{ withCredentials: true }).subscribe({
      next:(res:any)=>{
      this.cancelEdit();
      window.location.reload();
      alert("修改成功")},
...
downloadExcel(): void {
  ...
  this.http.post(`http://localhost:8080/OrderShipment/0210/downloadExcel?excelType=${type}`,data,{withCredentials: true, responseType: 'blob' as const}).subscribe(blob => {
...
// 選到的檔案
   ...
   uploadExcel(): void {
     ...
     this.http.post('http://localhost:8080/OrderShipment/0210/importExcel', formData, {  withCredentials: true,responseType: 'text' })
       .subscribe({
         next: (res: string) => {
         alert('上傳檔案成功!!');
         },
...
downloadPdf() {
  ...
  this.http.post('http://localhost:8080/OrderShipment/0210/pdf',data,{responseType: 'blob',withCredentials: true}).subscribe(blob => {
    const url = window.URL.createObjectURL(blob);
